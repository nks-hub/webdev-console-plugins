...
public sealed class MySqlModule : IServiceModule, IAsyncDisposable
{
    ...
    public Task<ServiceStatus> GetStatusAsync(CancellationToken ct)
    {
        ...
        lock (_stateLock)
        {
            ...
            // tracked Process has exited (user ran `pkill -f mysqld`, OS OOM,
            // crash that Process.Exited didn't reliably surface…), flip state
            // to Crashed so the UI stops showing a stale pid/uptime. The
            // clean StopAsync path sets _state=Stopped before disposal so it
