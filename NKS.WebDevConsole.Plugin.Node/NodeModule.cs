...
public sealed class NodeModule : IServiceModule, IAsyncDisposable
{
    ...
    private void DetectNodeExecutable()
    {
        ...
        if (Directory.Exists(nodeRoot))
        {
            ...
            // Semver-aware descending order so "20.5.0" sorts above "9.0.0".
            // Ordinal string compare would rank "9.0.0" first because '9' > '2'
            // in ASCII — fine for 1-digit majors, broken as soon as Node 10+
