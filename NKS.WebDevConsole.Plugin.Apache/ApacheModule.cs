...
public sealed class ApacheModule : IServiceModule, IAsyncDisposable
{
    ...
    private async Task GenerateMainConfigAsync(CancellationToken ct)
    {
        ...
        await EnsureDefaultVhostAsync(ct);
    ...
    public async Task GenerateVhostAsync(SiteConfig site, CancellationToken ct = default)
    {
        ...
        var outPath = ResolveVhostPath(site);
        await File.WriteAllTextAsync(outPath, result, ct);
        foreach (var stalePath in ResolveStaleVhostPaths(site.Domain, outPath))
        {
    ...
    public Task RemoveVhostAsync(string domain, CancellationToken ct = default)
    {
        ...
        var previousLocalhostPath = Path.GetFullPath(Path.Combine(baseDir, "000-localhost.conf"));
        if (string.Equals(domain, "localhost", StringComparison.OrdinalIgnoreCase)
            && !string.Equals(primaryPath, previousLocalhostPath, StringComparison.OrdinalIgnoreCase)
            && File.Exists(previousLocalhostPath))
        {
            File.Delete(previousLocalhostPath);
            _logger.LogInformation("Removed vhost for {Domain}", domain);
        }
    ...
    private string ResolveVhostPath(SiteConfig site)
    {
        var fileName = string.Equals(site.Domain, "localhost", StringComparison.OrdinalIgnoreCase)
            ? (GetLocalhostLoopbackEnabled(site) ? "000-localhost.conf" : "010-localhost.conf")
            : $"{site.Domain}.conf";
    ...
    private IEnumerable<string> ResolveAllVhostPaths(string domain)
    {
        if (string.Equals(domain, "localhost", StringComparison.OrdinalIgnoreCase))
        {
            yield return Path.GetFullPath(Path.Combine(_config.VhostsDirectory, "000-localhost.conf"));
            yield return Path.GetFullPath(Path.Combine(_config.VhostsDirectory, "010-localhost.conf"));
        }

        yield return Path.GetFullPath(Path.Combine(_config.VhostsDirectory, $"{domain}.conf"));
    }

    private IEnumerable<string> ResolveStaleVhostPaths(string domain, string activePath)
    {
        var activeFullPath = Path.GetFullPath(activePath);
        foreach (var candidate in ResolveAllVhostPaths(domain))
        {
            if (!string.Equals(candidate, activeFullPath, StringComparison.OrdinalIgnoreCase))
                yield return candidate;
        }
    }

    private async Task EnsureDefaultVhostAsync(CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(_config.VhostsDirectory))
            return;
        ...
        {
            "# Generated by NKS WebDev Console - default unmatched Apache vhost.",
            "# Keeps raw IP / unknown Host requests from falling through to a real site.",
            $"<VirtualHost *:{_config.HttpPort}>",
            "    ServerName wdc-unmatched.local",
            "    <Location />",
            "        Require all denied",
            "    </Location>",
            "</VirtualHost>",
        ...
        var localhostSslDir = Path.Combine(WdcPaths.SslRoot, "sites", "localhost");
        var certPath = Path.Combine(localhostSslDir, "cert.pem");
        var keyPath = Path.Combine(localhostSslDir, "key.pem");
        if (File.Exists(certPath) && File.Exists(keyPath))
        {
            lines.AddRange(new[]
            {
                $"<VirtualHost *:{_config.HttpsPort}>",
                "    ServerName wdc-unmatched.local",
                "    SSLEngine on",
                $"    SSLCertificateFile \"{ApachePath(certPath)}\"",
                $"    SSLCertificateKeyFile \"{ApachePath(keyPath)}\"",
                "    <Location />",
                "        Require all denied",
                "    </Location>",
                "</VirtualHost>",
        ...
        var path = Path.Combine(_config.VhostsDirectory, "005-default.conf");
        await File.WriteAllLinesAsync(path, lines, ct);
        var previousPath = Path.Combine(_config.VhostsDirectory, "000-default.conf");
    ...
    private static string[] GetBindAddresses(SiteConfig site)
    {
        try
        {
            if (site.GetType().GetProperty("BindAddresses")?.GetValue(site) is IEnumerable<string> values)
            {
                var normalized = NormalizeConfiguredBindScopes(values, GetBindAddress(site));
        ...
        {
            // Older SDK/Core versions may not expose BindAddresses.
        }

        return NormalizeConfiguredBindScopes(null, GetBindAddress(site));
    }

    private static bool GetLocalhostLoopbackEnabled(SiteConfig site)
    {
        try
        {
            return site.GetType().GetProperty("LocalhostLoopbackEnabled")?.GetValue(site) as bool? ?? false;
        }
        catch
        {
            return false;
    ...
    {
        var input = (bindAddresses ?? Array.Empty<string>())
            .Where(v => !string.IsNullOrWhiteSpace(v))
            .Select(NormalizeBindScope)
            .ToArray();
        if (input.Length == 0)
            input = [NormalizeBindScope(fallback)];
    ...
    public Task<ServiceStatus> GetStatusAsync(CancellationToken ct)
    {
        ...
        lock (_stateLock)
        {
            ...
            // tracked Process has exited (user ran `pkill -f httpd`, OS OOM,
            // segfault that Process.Exited didn't reliably surface…), flip
            // state to Crashed so the UI stops showing a stale pid/uptime.
            // StopAsync sets _state=Stopped before disposing _process so the
            // clean-shutdown path is untouched by this block.
            if (_state is ServiceState.Running or ServiceState.Starting
