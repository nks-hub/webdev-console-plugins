...
public sealed class MariaDBModule : IServiceModule, IAsyncDisposable
{
    ...
    {
        _logger.LogInformation("MariaDB initialized. Binaries root: {Root}", _config.BinariesRoot);
        ...
        if (_config.ApplyOwnBinaryDefaults())
        {
            ...
            // Emit a default my.cnf if none exists. Without it mariadbd aborts
            // on startup with "Could not open required defaults file". Mirrors
            // MySqlModule's GenerateDefaultIni — portable port/datadir/socket
            // pointing at our managed tree, skip-grant-tables for dev-mode
            // password-less root, bind loopback for safety.
            await EnsureDefaultConfigAsync(ct);

            // Initialize the datadir on first run (mariadb-install-db seeds the
            // mysql.* system tables). Skipped if already populated.
    ...
    /// <summary>Generate a default my.cnf when none is configured, so mariadbd
    /// has somewhere to load --defaults-file from. Idempotent.</summary>
    private async Task EnsureDefaultConfigAsync(CancellationToken ct)
    {
        var cfgPath = string.IsNullOrEmpty(_config.ConfigFile)
            ? Path.Combine(_config.DataDir, "my.ini")
            : _config.ConfigFile;
        _config.ConfigFile = cfgPath;
        if (File.Exists(cfgPath)) return;

        var dataDir = _config.DataDir.Replace('\\', '/');
        var logDir = _config.LogDirectory.Replace('\\', '/');
        var content = $@"# Generated by NKS WebDev Console — do not edit manually
[mysqld]
port={_config.Port}
...
";
        ...
        await File.WriteAllTextAsync(cfgPath, content, ct);
        _logger.LogInformation("Generated default my.ini at {Path}", cfgPath);
    }

    /// <summary>Run mariadb-install-db once so the mysql.* system tables exist.
    /// Idempotent — detects a previously-seeded datadir via mysql/user.* files.</summary>
    private async Task EnsureDatadirInitializedAsync(CancellationToken ct)
    {
        var mysqlSys = Path.Combine(_config.DataDir, "mysql");
        if (Directory.Exists(mysqlSys) && Directory.EnumerateFiles(mysqlSys).Any())
        ...
        // scripts/mariadb-install-db is a peer of bin/ under <prefix>/
        var installDb = Path.Combine(Path.GetDirectoryName(binDir)!, "scripts", "mariadb-install-db");
        if (!File.Exists(installDb))
        {
            _logger.LogWarning("mariadb-install-db not found next to {Exe}; datadir not seeded", _config.ExecutablePath);
            return;
        }

        var args = new List<string>
        {
            $"--defaults-file={_config.ConfigFile}",
            $"--datadir={_config.DataDir}",
            $"--user={Environment.UserName}",
            "--auth-root-authentication-method=normal",
        };
        _logger.LogInformation("Seeding MariaDB datadir: {Exe} {Args}", installDb, string.Join(' ', args));
        var result = await _runner.RunAsync(installDb, args, null, ct);
        if (result.ExitCode != 0)
        {
            _logger.LogWarning(
                "mariadb-install-db exited {Code}. stderr: {Err}",
                result.ExitCode,
                (result.StandardError ?? string.Empty).Trim());
        }
    }
    ...
    public Task<ServiceStatus> GetStatusAsync(CancellationToken ct)
    {
        ...
        lock (_stateLock)
        {
            ...
            // tracked Process has exited (user ran `pkill -f mariadbd`, OS
            // OOM, crash that Process.Exited didn't reliably surface…), flip
            // state to Crashed so the UI stops showing a stale pid/uptime.
            // The clean StopAsync path sets _state=Stopped before disposal
